...
// Load backend/.env into process.env before anything reads it. Zero-dependency
// and works on any Node version, so `OPENAI_API_KEY=...` in backend/.env just
// works with `npm start` (without needing to `export` it each time).
(function loadDotEnv() {
  try {
